...
pub async fn get_report_table(deps: Vec<Dependency>, m: &MultiProgress) -> ResultTable {
    let pb = Arc::new(Mutex::new(
        m.add(ProgressBar::new(deps.len().try_into().unwrap_or(0))),
    ));

    let spinner_style = ProgressStyle::default_spinner().tick_chars("⠁⠂⠄⡀⢀⠠⠐⠈ ");
    pb.lock().unwrap().set_style(spinner_style);
    pb.lock()
        .unwrap()
        .set_prefix(format!("{}", "[3/3]".bold().dimmed()));

    let deps = deps.into_iter().map(|mut dep| {
        let pb = Arc::clone(&pb);
        async move {
            pb.lock().unwrap().set_message(format!("{} {}", "Fetching".dimmed(), dep.get_name()));
            let url = format!("{}{}", NPM_URL, dep.get_name());
            let resp = reqwest::get(&url).await.expect("Cannot send request");
